...
@njit
def find_best_split_for_feature_fraction_le(x, sort_ids, group_ids, group_y, raw_point_indices, q,
                                        group_sizes,
...
@njit
def find_best_split_for_feature_fraction_gt(x, sort_ids, group_ids, group_y, raw_point_indices, q,
                                           group_sizes,
                                           total_sum, total_count,
                                           init_k, init_min_k,
                                           fraction_threshold):
...
class FractionTreeBuilder:
    ...
    def find_best_split(self, data: TreeData, point_indices):
        ...
        fraction_threshold_grid = np.linspace(0.0, 1.0, 10)
        ...
        for feature_id in self.feature_iterator(data):
            ...
            for fraction_threshold in fraction_threshold_grid:
                ...
                split_any_le, loss_any_le = find_best_split_for_feature_fraction_le(
                    data.X[:, feature_id],
                    sort_ids,
                    data.group_ids,
                    data.y,
    ...
    def reorder_indices(self, point_indices: np.ndarray, left: int, right: int,
                        data: TreeData, split: FractionSplitInfo) -> int:
    ...
    def convert_to_tree(self, nodes: List[Node]) -> FractionTree:
        n_nodes = len(nodes)
        tree = FractionTree(
    ...
    def build(self, X, y, group_sizes) -> FractionTree:
...
class FractionSetInputTree(BaseEstimator):
    def __init__(self, max_depth: int | None = None, min_samples_leaf: int = 2, extra_rand: bool = False,
    ...
    def _predict_one_group(self, group_x):
        ...
        while self.tree_.left[node_id] != self.tree_.right[node_id]:
            ...
            if mode == ThresholdMode.ANY.value:
                ...
                is_left = (
                    np.mean(group_x[:, feature] <= threshold) >= self.tree_.fraction_threshold[node_id]
                )
